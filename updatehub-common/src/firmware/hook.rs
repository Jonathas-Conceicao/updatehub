...
pub fn run_hook(path: &Path) -> Result<String, failure::Error> {
    if !path.exists() {
        return Ok("".into());
    }
...
pub fn run_hooks_from_dir(path: &Path) -> Result<MetadataValue, failure::Error> {
...
pub fn run_script(cmd: &str) -> Result<String, failure::Error> {
